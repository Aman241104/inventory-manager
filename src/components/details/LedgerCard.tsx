    ...
    <div className="bg-white border-2 border-slate-900 mb-8 print-break-inside-avoid ledger-border font-sans">
      ...
      <div className="flex justify-between items-center p-3 border-b-2 border-slate-900 ledger-border-b font-bold print-text-black">
        <div className="uppercase">
          {productName} 
          <span className="text-xs ml-2 text-slate-500 print-text-black">({unitType})</span>
        </div>
      ...
      <div className="flex min-h-[300px]">
        ...
        {/* Left Side: Purchase */}
        <div className="w-1/2 p-4 border-r-2 border-slate-900 ledger-border-r flex flex-col print-text-black">
          <div className="text-xs font-bold uppercase underline mb-4 text-slate-500 print-text-black">Purchase Details</div>
          
          <div className="flex-1 space-y-4">
            <div>
              <div className="text-xl font-black">{purchasedQty} <span className="text-sm">QTY PURCHASED</span></div>
            </div>
            ...
            <div className="mt-8 pt-4 border-t border-dashed border-slate-300">
              ...
              <div className="h-4 border-b border-slate-300 w-3/4 mt-4"></div>
            </div>
          </div>
        </div>

        {/* Right Side: Sales */}
        <div className="w-1/2 p-4 flex flex-col print-text-black">
                ...
                {sales.length > 0 ? (
                  sales.map((sale: any) => (
                    <tr key={sale.saleId} className="border-b border-slate-100 last:border-0">
                      <td className="py-1 uppercase text-slate-600 truncate max-w-[150px] print-text-black">
                        {sale.customerName}
                      </td>
      ...
      <div className="flex border-t-2 border-slate-900 ledger-border-t font-bold text-lg print-text-black bg-slate-50 print:bg-white">
        <div className="w-1/2 p-3 border-r-2 border-slate-900 ledger-border-r uppercase">
          {remainingQty} Balance Day End
        </div>
        <div className="w-1/2 p-3 uppercase text-right">
          (Total {totalSoldQty})
        </div>
      </div>
    </div>
